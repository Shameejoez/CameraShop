...
describe('Catalog-sort', () => {
  it('Should render catalog-sort', async() => {
    browserHistory.push(AppRoutes.Catalog);
    render(fakeApp);

    await screen.findByText('по цене');
    await screen.findByText('по популярности');

    await screen.findByRole('radio', {name: 'По возрастанию'});
    await screen.findByRole('radio', {name: 'По убыванию'});
  });
});
